import streamlit as st
import matplotlib.pyplot as plt
import matplotlib.patches as patches
import math
import random
import pandas as pd

# ตั้งค่าหน้าเว็บ
st.set_page_config(page_title="Ultrasonic Design Master", page_icon="⚙️", layout="wide")

# --- Language Settings (เมนูเปลี่ยนภาษา) ---
st.sidebar.header("🌐 Language / ภาษา")
lang_choice = st.sidebar.radio(
    "", 
    ["🇹🇭 ภาษาไทย", "🇬🇧 English"], 
    index=0,
    label_visibility="collapsed"
)
lang = "th" if "ไทย" in lang_choice else "en"

# --- Translations Dictionary (ฐานข้อมูลคำแปล) ---
T = {
    "title": {
        "th": "⚙️ เครื่องมือออกแบบ Ultrasonic Cleaner (Master Edition)",
        "en": "⚙️ Ultrasonic Cleaner Design Tool (Master Edition)"
    },
    "caption": {
        "th": "🚀 คำนวณตามมาตรฐานวิศวกรรม | 📘 ฐานข้อมูลวิจัยฉบับสมบูรณ์",
        "en": "🚀 Engineering Standard Calculation | 📘 Complete Research Database"
    },
    "nav_header": {"th": "เมนูเลือกหน้า", "en": "Navigation"},
    "nav_calc": {"th": "📟 โปรแกรมคำนวณ", "en": "📟 Calculator"},
    "nav_manual": {"th": "📘 คู่มือและความรู้", "en": "📘 Knowledge Base & Manual"},
    
    # Input Labels
    "tank_header": {"th": "1. ข้อมูลถัง (Tank Info)", "en": "1. Tank Dimensions"},
    "L": {"th": "ความยาว (cm)", "en": "Length (cm)"},
    "W": {"th": "ความกว้าง (cm)", "en": "Width (cm)"},
    "H": {"th": "ความสูงถัง (cm)", "en": "Tank Height (cm)"},
    "level": {"th": "ระดับน้ำใช้งาน (cm)", "en": "Water Level (cm)"},
    
    "cond_header": {"th": "2. เงื่อนไขการใช้งาน", "en": "2. Usage Conditions"},
    "chem": {"th": "ใช้น้ำยาเคมี/กรด (Chemistry)", "en": "Use Chemistry/Acid"},
    "chem_help": {"th": "ช่วยลดความต้องการพลังงานลง เพราะเคมีช่วยกัดคราบ", "en": "Reduces power requirement as chemicals help loosen contaminants"},
    "heavy": {"th": "ชิ้นงานหนาแน่น (Heavy Load)", "en": "Heavy Mass Load"},
    "heavy_help": {"th": "เพิ่มกำลัง 15% ชดเชยการดูดซับเสียงของทองแดง", "en": "Adds 15% power to compensate for copper's sound absorption"},
    
    "spec_header": {"th": "3. สเปกบอร์ด (Hardware Specs)", "en": "3. Hardware Specs"},
    "w_board": {"th": "W/บอร์ด", "en": "Watts/Board"},
    "h_board": {"th": "หัว/บอร์ด", "en": "Heads/Board"},
    
    "design_header": {"th": "🛠️ คำนวณออกแบบระบบ", "en": "🛠️ System Design Calculation"},
    "mode_new": {"th": "✨ ออกแบบใหม่ (Design New)", "en": "✨ Design New System"},
    "mode_check": {"th": "🔍 ตรวจสอบของที่มี (Check Existing)", "en": "🔍 Check Existing System"},
    "rec_val": {"th": "💡 ค่าแนะนำ", "en": "💡 Recommended"},
    "target": {"th": "🎯 กำหนดความแรงเป้าหมาย (Target W/L)", "en": "🎯 Target Power Density (W/L)"},
    "ratio": {"th": "สัดส่วนคลื่น 28kHz", "en": "28kHz Ratio"},
    
    "qty_exist": {"th": "จำนวนบอร์ดที่มีอยู่", "en": "Existing Board Qty"},
    
    # Results
    "vol": {"th": "💧 ปริมาตรน้ำ", "en": "💧 Water Volume"},
    "p_total": {"th": "⚡ กำลังไฟรวม", "en": "⚡ Total Power"},
    "density": {"th": "📊 ความหนาแน่นจริง", "en": "📊 Actual Density"},
    "analysis": {"th": "📝 ผลการวิเคราะห์", "en": "📝 Analysis Result"},
    "pass": {"th": "✅ **ผ่านเกณฑ์มาตรฐาน**", "en": "✅ **PASSED Standard**"},
    "fail": {"th": "❌ **พลังงานต่ำกว่าเกณฑ์**", "en": "❌ **BELOW Standard**"},
    "fail_msg": {"th": "ขาดอีก", "en": "Missing"},
    "large_tank": {"th": "ถังขนาดใหญ่ (>100L) อาจใช้ค่า W/L ต่ำกว่านี้ได้ (Large Tank Phenomenon)", "en": "Large tanks (>100L) may require lower W/L (Large Tank Phenomenon)"},
    "bom_header": {"th": "📦 รายการอุปกรณ์ (BOM)", "en": "📦 Bill of Materials"},
    "layout": {"th": "📍 ผังการจัดวาง (Layout Simulation)", "en": "📍 Transducer Layout Simulation"},
    "mount": {"th": "มุมมองการติดตั้ง:", "en": "Mounting View:"},
    "bottom": {"th": "ก้นถัง (Bottom)", "en": "Bottom"},
    "side": {"th": "ข้างถัง (Side)", "en": "Side Wall"},
    "warn_bottom": {"th": "⚠️ อย่าลืม: ห้ามวางชิ้นงานทับหัวโดยตรง ต้องมีตะแกรงยกสูง", "en": "⚠️ Note: Never place parts directly on transducers. Use a basket."},
    "warn_side": {"th": "✅ การติดข้างถังช่วยลดปัญหาตะกอนทับหัว แต่ต้องระวังจุดบอดตรงกลาง", "en": "✅ Side mounting prevents sludge buildup but watch out for blind spots."}
}

def t(key):
    return T[key][lang]

# --- Helper Functions ---
def get_recommended_density(vol_liters, has_chem, heavy_load):
    # Based on Research Data
    if vol_liters <= 10: base_wl = 35.0
    elif vol_liters <= 20: base_wl = 30.0
    elif vol_liters <= 50: base_wl = 25.0
    elif vol_liters <= 100: base_wl = 20.0
    elif vol_liters <= 190: base_wl = 10.0
    else: base_wl = 5.3 

    if has_chem: base_wl *= 0.7
    if heavy_load: base_wl *= 1.15
    return round(base_wl, 1)

def draw_tank(l, h_limit, h_list, title, side=False, tank_h=0, water_h=0, off=False):
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.set_title(title, fontsize=10, weight='bold')
    
    if side:
        ax.add_patch(patches.Rectangle((0,0), l, tank_h, fc='#eeeeee', ec='#444', lw=2))
        ax.add_patch(patches.Rectangle((0,0), l, water_h, fc='#b3e5fc', alpha=0.6))
        ax.axhline(y=water_h, color='#0277bd', linestyle='--', lw=1)
        area_h = water_h
    else:
        ax.add_patch(patches.Rectangle((0,0), l, h_limit, fc='#e1f5fe', ec='#444', lw=2))
        area_h = h_limit
    
    n=len(h_list)
    if n>0 and area_h>0:
        cols = math.ceil(math.sqrt(n*(l/area_h)))
        rows = math.ceil(n/cols)
        sp_x = l / (cols + 1)
        sp_y = area_h / (rows + 1)
        for r in range(rows):
            for c in range(cols):
                cnt = r*cols + c
                if cnt < n:
                    fq=h_list[cnt]
                    base_x = (c + 1) * sp_x
                    base_y = (r + 1) * sp_y
                    stagger = (sp_x/2) if (r%2!=0) else 0
                    offset_side = (sp_x/2) if off else 0
                    x = base_x + stagger + offset_side
                    if x > l - (sp_x/2): x = x - l + (sp_x/2)
                    y = base_y
                    c_node = '#d32f2f' if fq==28 else '#1976d2'
                    ax.add_patch(plt.Circle((x,y), 2.5, color=c_node, ec='white', alpha=0.9))
                    ax.text(x,y, str(fq), color='white', ha='center', va='center', fontsize=7, weight='bold')
    ax.set_xlim(-2, l+2); ax.set_ylim(-2, (tank_h if side else h_limit)+2)
    ax.set_aspect('equal')
    return fig

# --- Main App Logic ---
st.title(t("title"))
st.caption(t("caption"))

page = st.sidebar.radio(t("nav_header"), [t("nav_manual"), t("nav_calc")])
st.sidebar.divider()

if page == t("nav_manual"):
    if lang == "th":
        # --- THAI MANUAL ---
        st.header("📘 องค์ความรู้และการออกแบบ (Engineering Manual)")
        tab0, tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
            "📖 คู่มือการใช้โปรแกรม", "1. ทฤษฎี & ความถี่", "2. มาตรฐาน W/L", 
            "3. การติดตั้ง & Safety", "4. การทดสอบ (Foil Test)", "5. สูตรคำนวณ", "📝 ข้อมูลวิจัย"
        ])
        
        with tab0:
            st.markdown("""
            ### 📖 วิธีใช้งานโปรแกรม
            **1. ไปที่หน้าคำนวณ:** เลือกเมนู **"📟 โปรแกรมคำนวณ"** ด้านซ้าย
            **2. กรอกข้อมูลถัง:** ใส่กว้าง x ยาว x สูง และระดับน้ำ
            **3. เลือกเงื่อนไข:** * **ใช้น้ำยาเคมี:** ติ๊ก ✅ (โปรแกรมลดความแรงให้ เพราะเคมีช่วยกัดคราบ)
            * **ชิ้นงานหนาแน่น:** ติ๊ก ✅ (โปรแกรมเพิ่มความแรงให้ ชดเชยการดูดซับเสียง)
            **4. ระบุสเปกบอร์ด:** ใส่ Watt และจำนวนหัวต่อบอร์ด
            **5. เลือกโหมด:**
            * **✨ ออกแบบใหม่:** ใส่ความแรงที่อยากได้ -> คำนวณจำนวนบอร์ด
            * **🔍 ตรวจสอบ:** ใส่บอร์ดที่มี -> บอกว่าแรงพอไหม
            """)
        with tab1:
            st.markdown("""
            ### 🌊 Acoustic Cavitation & Frequency
            **28 kHz:** ฟองใหญ่ แรงระเบิดสูง เหมาะกับ **"ระเบิด" คราบฟลักซ์หนาๆ** แต่เสียงดัง
            **40 kHz:** ฟองเล็ก เข้าถึงซอกมุมได้ดี อ่อนโยนกว่า
            **สรุป:** 28kHz เน้นงานหนัก (Roughing), 40kHz เน้นงานละเอียด (Finishing)
            """)
        with tab2:
            st.markdown("### 📊 ความสำคัญของ Watts per Liter (W/L)")
            df_std = pd.DataFrame({
                "ขนาดถัง": ["10 L", "20 L", "50 L", "100 L", "> 190 L"],
                "ค่าแนะนำ (W/L)": ["30-35", "25-30", "20-25", "15-20", "~5.3"]
            })
            st.table(df_std)
        with tab3:
            st.markdown("""
            ### 🛠️ การติดตั้ง
            **1. ติดก้นถัง:** คลื่นทั่วถึง แต่ตะกอนอาจทับหัว
            **2. ติดข้างถัง:** อายุยืนยาว แต่ต้องระวังจุดบอด
            **⚠️ ข้อควรระวัง:** ห้ามวางงานก้นถัง (ต้องใช้ตะแกรง), ห้ามเดินเครื่องเปล่า (ต้องมีน้ำ)
            """)
        with tab4:
            st.markdown("### 🧪 Foil Test: จุ่มฟอยล์ 30-60 วิ ต้องเกิดรอยยับสม่ำเสมอ")
        with tab5:
            st.latex(r" W/L = \frac{W/Gal}{3.785} ")
            st.latex(r" P_{req} = V_{eff} \times D_{target} ")
        with tab6:
            st.info("📂 **ข้อมูลวิจัย:** ถังใหญ่ >190L ใช้ 5.3 W/L ก็พอ / งานทองแดงควรเผื่อกำลัง 10-15%")

    else:
        # --- ENGLISH MANUAL ---
        st.header("📘 Engineering Manual & Knowledge Base")
        tab0, tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
            "📖 User Guide", "1. Theory & Freq", "2. W/L Standards", 
            "3. Installation", "4. Foil Test", "5. Formulas", "📝 Research Notes"
        ])
        
        with tab0:
            st.markdown("""
            ### 📖 How to use
            **1. Go to Calculator:** Select **"📟 Calculator"** from sidebar.
            **2. Tank Info:** Enter L x W x H and Water Level.
            **3. Conditions:** * **Chemistry:** Check ✅ (Reduces power req.)
            * **Heavy Load:** Check ✅ (Increases power req.)
            **4. Hardware Specs:** Enter Watts and Heads per board.
            **5. Mode:**
            * **✨ Design New:** Input target W/L -> Get Board Qty.
            * **🔍 Check Existing:** Input Board Qty -> Get Pass/Fail result.
            """)
        with tab1:
            st.markdown("""
            ### 🌊 Acoustic Cavitation & Frequency
            **28 kHz:** Large bubbles, high impact. Best for **Heavy Flux Removal**.
            **40 kHz:** Small bubbles, high penetration. Best for fine cleaning/complex parts.
            """)
        with tab2:
            st.markdown("### 📊 Watts per Liter (W/L) Standards")
            df_std = pd.DataFrame({
                "Tank Size": ["10 L", "20 L", "50 L", "100 L", "> 190 L"],
                "Rec. W/L": ["30-35", "25-30", "20-25", "15-20", "~5.3"]
            })
            st.table(df_std)
        with tab3:
            st.markdown("""
            ### 🛠️ Mounting
            **1. Bottom:** Direct distribution, but prone to sludge overheating.
            **2. Side:** Longer life, sludge falls to bottom. Watch for blind spots.
            **⚠️ Safety:** Never place parts on bottom (Use basket), No dry running.
            """)
        with tab4:
            st.markdown("### 🧪 Foil Test: Dip foil for 30-60s. Look for uniform perforations.")
        with tab5:
            st.latex(r" W/L = \frac{W/Gal}{3.785} ")
            st.latex(r" P_{req} = V_{eff} \times D_{target} ")
        with tab6:
            st.info("📂 **Research:** Large tanks (>190L) need only 5.3 W/L. Copper mass load requires +10-15% power.")

elif page == t("nav_calc"):
    st.sidebar.header(t("tank_header"))
    # Fix: Added .0 to ensure float types
    L = st.sidebar.number_input(t("L"), value=60.0, step=1.0)
    W = st.sidebar.number_input(t("W"), value=40.0, step=1.0)
    H_tank = st.sidebar.number_input(t("H"), value=50.0, step=1.0)
    water_level = st.sidebar.number_input(t("level"), value=35.0, step=1.0)
    
    st.sidebar.header(t("cond_header"))
    use_chem = st.sidebar.checkbox(t("chem"), value=True, help=t("chem_help"))
    heavy_load = st.sidebar.checkbox(t("heavy"), value=True, help=t("heavy_help"))
    
    st.sidebar.header(t("spec_header"))
    c1, c2 = st.sidebar.columns(2)
    with c1:
        w_board_28 = st.number_input(f"{t('w_board')} (28k)", value=120.0, step=10.0)
        h_board_28 = st.number_input(f"{t('h_board')} (28k)", value=2, min_value=1)
    with c2:
        w_board_40 = st.number_input(f"{t('w_board')} (40k)", value=120.0, step=10.0)
        h_board_40 = st.number_input(f"{t('h_board')} (40k)", value=3, min_value=1)
        
    vol = (L * W * water_level) / 1000
    rec_density = get_recommended_density(vol, use_chem, heavy_load)
    
    st.subheader(t("design_header"))
    mode = st.radio("", [t("mode_new"), t("mode_check")], horizontal=True)
    st.divider()
    
    n_b28, n_b40 = 0, 0
    target_density = 0.0
    actual_density = 0.0
    
    if mode == t("mode_new"):
        c_in1, c_in2 = st.columns(2)
        with c_in1:
            st.info(f"{t('rec_val')}: **{rec_density} W/L**")
            target_density = st.number_input(t("target"), value=rec_density, step=0.5)
        with c_in2:
            ratio_28 = st.slider(f"{t('ratio')} (%)", 0, 100, 70) / 100
        
        total_p_req = vol * target_density
        p_28 = total_p_req * ratio_28
        p_40 = total_p_req * (1 - ratio_28)
        
        n_b28 = math.ceil(p_28 / w_board_28) if p_28 > 0 else 0
        n_b40 = math.ceil(p_40 / w_board_40) if p_40 > 0 else 0
        if p_40 > 0 and n_b40 == 0: n_b40 = 1
        
        real_total_w = (n_b28 * w_board_28) + (n_b40 * w_board_40)
        actual_density = real_total_w / vol if vol > 0 else 0
        
    else:
        st.warning(f"{t('rec_val')}: **{rec_density} W/L**")
        c_ex1, c_ex2 = st.columns(2)
        with c_ex1:
            n_b28 = st.number_input(f"{t('qty_exist')} (28k)", value=3, min_value=0)
        with c_ex2:
            n_b40 = st.number_input(f"{t('qty_exist')} (40k)", value=1, min_value=0)
            
        real_total_w = (n_b28 * w_board_28) + (n_b40 * w_board_40)
        actual_density = real_total_w / vol if vol > 0 else 0
        target_density = rec_density

    n_h28 = int(n_b28 * h_board_28)
    n_h40 = int(n_b40 * h_board_40)
    
    m1, m2, m3 = st.columns(3)
    m1.metric(t("vol"), f"{vol:.2f} L")
    m2.metric(t("p_total"), f"{real_total_w:.0f} W")
    m3.metric(t("density"), f"{actual_density:.2f} W/L", delta=f"{actual_density - target_density:.2f}")
    st.markdown("---")
    
    c_an1, c_an2 = st.columns([2, 1])
    with c_an1:
        st.subheader(t("analysis"))
        if actual_density >= (target_density * 0.95):
            st.success(f"{t('pass')} ({actual_density:.2f} W/L)")
        else:
            st.error(f"{t('fail')} ({t('fail_msg')} {target_density - actual_density:.1f} W/L)")
            if vol > 100:
                st.info(f"💡 {t('large_tank')}")
            
    with c_an2:
        st.markdown(f"""
        <div style="background-color:#e3f2fd; padding:15px; border-radius:10px; border:1px solid #90caf9; color: #000000;">
            <h4 style="margin:0; color:#0d47a1;">{t("bom_header")}</h4>
            <hr style="margin:5px 0; border-top: 1px solid #1565c0;">
            <p style="margin:0; font-size:16px;"><b>🔴 28 kHz:</b> {n_b28} <span style="font-size:14px; color:#333;">(= {n_h28})</span></p>
            <br>
            <p style="margin:0; font-size:16px;"><b>🔵 40 kHz:</b> {n_b40} <span style="font-size:14px; color:#333;">(= {n_h40})</span></p>
        </div>
        """, unsafe_allow_html=True)

    st.subheader(t("layout"))
    mount_opt = st.radio(t("mount"), [t("bottom"), t("side")], horizontal=True)
    heads_list = [28]*n_h28 + [40]*n_h40
    random.seed(42); random.shuffle(heads_list)
    
    if mount_opt == t("bottom"):
        st.pyplot(draw_tank(L, W, heads_list, "Bottom View"))
        st.caption(t("warn_bottom"))
    else:
        mid = len(heads_list)//2
        g1, g2 = st.columns(2)
        g1.pyplot(draw_tank(L, water_level, heads_list[:mid], "Side A", True, H_tank, water_level))
        g2.pyplot(draw_tank(L, water_level, heads_list[mid:], "Side B", True, H_tank, water_level, True))
        st.caption(t("warn_side"))
