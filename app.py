...
# ==========================================
# 1. LANGUAGE SETTINGS & TRANSLATIONS
# ==========================================
...
# ฟังก์ชันดึงข้อความตามภาษา (Dictionary)
# ผมคงข้อความไทยเดิมของคุณไว้ 100% ครับ
T = {
    "title": {
    ...
    # Input Labels
    "tank_info":  {"th": "1. ข้อมูลถัง (Tank Dimensions)", "en": "1. Tank Dimensions"},
    ...
    "cond_info":  {"th": "2. เงื่อนไขการใช้งาน (Conditions)", "en": "2. Usage Conditions"},
    ...
    "spec_info":  {"th": "3. สเปกบอร์ด (Hardware Specs)", "en": "3. Hardware Specs"},
    ...
    "mode_select": {"th": "เลือกโหมด:", "en": "Select Mode:"},
    ...
    "exist_check": {"th": "ℹ️ กำลังเปรียบเทียบกับค่าแนะนำ", "en": "ℹ️ Comparing with recommendation"},
...
# GEMINI AI ASSISTANT SECTION (เพิ่มส่วนถาม AI)
with st.sidebar.expander("🤖 ถามผู้ช่วย AI (Ask Gemini)", expanded=False):
    st.markdown("หากสงสัยวิธีใช้งาน กดปุ่มด้านล่างเพื่อถาม AI Gemini")
    st.link_button("↗️ เปิด Gemini", "https://gemini.google.com/app")
    st.info("**เคล็ดลับ:** ก๊อปปี้ข้อความนี้ไปวางถาม Gemini ได้เลย 👇")
    st.code("ฉันกำลังใช้โปรแกรมออกแบบ Ultrasonic Cleaner ต้องการคำแนะนำเรื่อง [ใส่เรื่องที่คุณสงสัย เช่น การเลือกค่า W/L หรือ การวางตำแหน่งหัว] ช่วยอธิบายหน่อย", language=None)
...
if page == t("nav_manual"):
    # เราใช้ Logic แยกภาษาตรงนี้ เพื่อให้จัดการข้อความยาวๆ ได้ง่าย
    if lang == "th":
        st.header("📘 องค์ความรู้และการออกแบบ (Engineering Manual)")
        tab0, tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
            "📖 คู่มือการใช้โปรแกรม", "1. ทฤษฎี & ความถี่", "2. มาตรฐาน W/L", 
            "3. การติดตั้ง & Safety", "4. การทดสอบ (Foil Test)", "5. สูตรคำนวณ", "📝 ข้อมูลเพิ่มเติม"
        ])
    ...
    else: # ENGLISH VERSION (แปลให้แล้วครับ)
...
# ==========================================
    ...
    st.sidebar.header(t("tank_info"))
    # ใช้ค่า Default ที่คุณกำหนดมา (L=170, W=80, Level=10)
    ...
    mode = st.radio(t("mode_select"), [t("mode_new"), t("mode_check")], horizontal=True)
    st.divider()
    ...
    if mode == t("mode_new"):
        ...
        st.warning(f"{t('exist_check')}: **{rec_density} W/L**")
